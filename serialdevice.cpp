...
bool SerialDevice::OpenConnection()
{
    ...
    {
        status = 3;
        connect(activePort,SIGNAL(error(QSerialPort::SerialPortError))
...
void SerialDevice::HandleBytesWritten(qint64 bytesWritten)
{
    ...
    if (bytesWritten == -1)
    {
        qWarning() << QObject::tr("Failed to write data to port %1, error %2").arg(activePort->portName()).arg(activePort->errorString()) << endl;
        status = 0;
        return;
    }
    else if (bytesWritten != writeCount)
    {
        qWarning() << QObject::tr("Failed to write all the data to port %1, error: %2").arg(activePort->portName()).arg(activePort->errorString()) << endl;
        status = 0;
        return;
    }
